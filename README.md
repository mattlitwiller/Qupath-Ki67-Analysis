...
8. Export for Power BI: Used to export analysis results in a csv file for visualizations (e.g. with Power BI). Useful for quality assessment over time in a Levey-Jennings graph or other. Template for Levey-Jennings graph in Power BI can be found in ```cell_data > levey-jennings.pbit```
