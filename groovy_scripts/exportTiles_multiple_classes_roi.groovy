...
def classNames = ["germinal_center", "light_zone", "dark_zone", "mantle"]   // names of classes of interest (simply add more values to list to add more classes) - only non-overlapping classes should be added or the model will not be trained properly
double downsample = 4  // which downsampling level to use -> how much to downsample the patches
//original glass threshold 50
double glassThreshold = 40  // which threshold to use for tissue detection (lower value => more tissue included in mask)
double percentageThreshold = 0.6  // if a patch contains less than X amount of tissue, it should be neglected (range [0.0, 1.0])
int patchSize = 780  // generated patch size
int pixelOverlap = 64  // stride for which patches are generated
...
for (className in classNames) {
    ...
    def tileExporter = new TileExporter(imageData)
        ...
        .includePartialTiles(false)      // Define inclusion of partially annotated tiles
