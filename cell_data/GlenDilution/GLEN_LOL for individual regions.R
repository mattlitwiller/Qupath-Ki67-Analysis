...
#y_axis <- 'Positivity (%)'
#main_title <- 'DZ Positivity Dilution GLEN'
y_axis <- 'Mean Slide DAB'
main_title <- 'GC DAB Mean Dilution GLEN'

region_colors <- c("light_zone" = "#c0402d", "dark_zone" = "#65be4b", "mantle" = "#1a0068", "germinal_center" = "#3cc0fe")
run_shapes <- c(1, 2, 3, 16) # Shapes: 1 = circle, 2 = triangle, 3 = plus, 16 = filled circle
...
#Run4 only
data <- data[data$Run == 4,]
data <- data[data$Parent == "germinal_center",]
...
if(FALSE) {
  ...
  plot(
    ...
    pch = run_shapes[data$Run],            # All points will have pch = 16 (solid circle)
  ...
  # Add a second legend for shapes (runs)
  legend("topright", 
         legend = c(1,2,3,4), 
         pch = run_shapes, 
         title = "Run", 
...
#    labels = paste("LOL ", round(lol_values[[parent]]$y, 2)),  # Round to 2 decimals
#    pos = 4,          # Position the label to the right of the point
#    col = "black"  # Match label color with point color
#  )
#}
