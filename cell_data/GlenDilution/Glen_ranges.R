...
region_colors <- c("light_zone" = "#c0402d", "dark_zone" = "#65be4b", "mantle" = "#1a0068", "germinal_center" = "#3cc0fe")
...
x_range = range(c(0, 1))
y_range = range(c(0, 1.45))
stdev = "mean_stdev"
y_metric_printed = "DAB Mean"
if(y_metric == "Positivity"){
  stdev = "pos_stdev"
  y_metric_printed = "Positivity (%)"
}
...
parent_values <- c("germinal_center", "light_zone", "dark_zone")
...
if(FALSE) {
  # Plot with plotly to see all info
  p <- plot_ly(data, 
               x = ~Fraction, 
               y = ~get(y_metric), 
               type = "scatter", 
               mode = "markers", 
               marker = list(color = region_colors[data$Parent], size = 10), 
               text = ~paste("Region:", Parent, "<br>Slide:", Name, "<br>DAB Mean:", get(y_metric)), 
               hoverinfo = "text")
  
  # Customize layout
  p <- layout(p, 
              title = "DAB Mean Dilution GLEN", 
              xaxis = list(title = "Dilution Concentration"), 
              yaxis = list(title = "Mean Slide DAB"))
  
  # Display the interactive plot
  p
}
  
data <- data[data$Parent != "mantle", ]
...
# Define a function to calculate the weighted standard deviation
weighted_stdev <- function(x, w, weighted_mean) {
...
# Loop through each combination of Parent and Fraction
for (parent in unique(data$Parent)) {
  for (fraction in unique(data$Fraction)) {
    
    # Subset data for the current Parent and Fraction combination
    subset_data <- data[data$Parent == parent & data$Fraction == fraction, ]
    
    # Weighted averages for Mean and Positivity
    weighted_mean <- weighted_avg(subset_data$Mean, subset_data$Count)
    weighted_positivity <- weighted_avg(subset_data$Positivity, subset_data$Count)
    
    # Weighted standard deviations for Mean and Positivity
    mean_stdev <- 2 * weighted_stdev(subset_data$Mean, subset_data$Count, weighted_mean)
    pos_stdev <- 2 * weighted_stdev(subset_data$Positivity, subset_data$Count, weighted_positivity)
    
    # Sum of Counts
    total_count <- sum(subset_data$Count)
    
    # Create a row for the agg_data dataframe
    agg_data <- rbind(agg_data, data.frame(
      Parent = parent,
      Fraction = fraction,
      Mean = weighted_mean,
      Positivity = weighted_positivity,
      Count = total_count,
...
combined_data <- agg_data[complete.cases(agg_data), ]
...
plot(
  ...
  main = "DAB Mean Dilution GLEN",
  xlim = x_range,
  ylim = range(combined_data[[y_metric]])
)
...
for(parent in parent_values) {
  ...
  region_data <- combined_data[combined_data$Parent == parent, ]
  
  # Set up a basic plot for the germinal_center region
  plot(region_data$Fraction, region_data[[y_metric]], 
       pch=19, col=region_data$Color, 
       xlab="Fraction", ylab=y_metric_printed, 
       ylim = range(c(0, max(region_data[[y_metric]] + 2*region_data[[stdev]]))),
       xlim = range(c(0,1)),
       main=paste("Glen Hospital", 
                  gsub("(\\b[a-z])", "\\U\\1", gsub("_", " ", parent), perl = TRUE),
                  y_metric_printed, "Dilution Curve"))
  
  
  # Add polygons (shading) for the stdev ranges
  # We'll shade based on the 'Stdev' values corresponding to the fractions
  for (i in 1:nrow(region_data)) {
    x_left <- region_data$Fraction[i] - 0.01  # Slightly smaller range to show overlap
    x_right <- region_data$Fraction[i] + 0.01
    y_bottom <- region_data[[y_metric]][i] - region_data[[stdev]][i]
    y_top <- region_data[[y_metric]][i] + region_data[[stdev]][i]
    
    # Draw polygon (shading)
    polygon(c(x_left, x_left, x_right, x_right), 
            c(y_bottom, y_top, y_top, y_bottom), 
            col=adjustcolor(region_data$Color[i], alpha.f = 0.2), border=NA)
  }
  
  # Overlay the points again (in case they get hidden by the shading)
  points(region_data$Fraction, region_data[[y_metric]], 
         pch=19, col=region_data$Color)
  ...
  # Calculate R-squared
  predicted_values <- predict(log_model)
  residuals <- parent_data[[y_metric]] - predicted_values
  SS_residual <- sum(residuals^2)
  SS_total <- sum((parent_data[[y_metric]] - mean(parent_data[[y_metric]]))^2)
  ...
  legend("topleft", 
         legend = c("95% Confidence across 3 runs", paste("Log Fit R-squared:", round(r_squared, 3))), 
         fill = c(adjustcolor(region_data$Color[1], alpha.f = 0.2), NA), 
         bty = "n", 
         border = c(1, NA),
         cex = 1)
