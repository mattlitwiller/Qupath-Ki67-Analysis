...
for (parent in unique_parents) {
  ...
  fit <- lm(Mean ~ Fraction, data = parent_data) 
  models[[parent]] <- fit
...
calculate_x <- function(parent, mean_value) {
  ...
  x_value <- (mean_value - intercept) / slope
  return(x_value)
}

# Apply the function to the combined_data dataframe
full_rows$x <- mapply(calculate_x, full_rows$Parent, full_rows$Mean)

# View the updated dataframe with the new x values
print(full_rows)

points(
  full_rows$x, full_rows$Mean,  # Plot the calculated x values and their corresponding Mean
...
for (parent in parent_values) {
  ...
  log_model <- nls(Mean ~ a + b * log(Fraction), 
                                      data = parent_data, 
  ...
  # Predict the Mean using the nls model
